...
4. 項目11(■返信3-青1)まで一文字も省略せず、最後まで書き切ること。` 
        },
        { 
          role: "user", 
          content: `
...
12. 【以上、全項目出力完了】`
      ...
      temperature: 0.7,
      max_tokens: 4000, // 文字数不足による停止を防ぐため上限をアップ
    });

    let resultText = completion.choices[0]?.message?.content || "";

    // --- 【物理的な強制処理：この順序を絶対に変えません】 ---
    
    // 1. まずスペースを根絶（全角・半角、文章中のあらゆる空白）
    resultText = resultText.replace(/[ 　]+/g, "");

    // 2. 相手の呼び名の間違いを修正
    resultText = resultText.replace(/○○くん|○○さん|あなた|君/g, "%send_nickname%");

    // 3. 【最重要】青1のチェック（スペース除去後の文字列で判定）
    // AIが「■返信3-青1」を出していなければ、強制的に差し込む
