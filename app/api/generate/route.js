...
export async function POST(req) {
  try {
    ...
    // ★追加：絵文字設定の動的判定
    const emojiInstruction = template?.emoji === "なし" 
      ? "【絵文字・記号は一切禁止】。非常にシンプルなテキストのみで構成せよ。" 
      : "【絵文字を多用せよ】🌈🍀🐱🍰🍹など、多彩な絵文字を各セリフに3個以上使い、女性らしく華やかにせよ。";
...
1. **絵文字方針**: ${emojiInstruction}
2. **体格制限**: 体重は必ず【43〜50kg】。
3. **アタック(1-3)の禁止事項**: 相手を呼ぶ言葉（%send_nickname%、あなた、君、○○さん、そちら等）を【1文字も使うな】。
   - 名前を呼ばずに「最近何してる？」「おすすめはある？」と質問せよ。
4. **返信(1-3)の必須事項**: 相手の呼び名は必ず【%send_nickname%】を使用せよ。
5. **URLの抹消**: インスタURL等のリンク行は、文字を入れず【完全な空行】にせよ。
6. **文章量**: すべてのセリフ（アタック・返信）を【3行以上】で構成せよ。
...
`;
    ...
    const response = await openai.chat.completions.create({
      ...
      temperature: 0.8, // 絵文字のバリエーションを出すため0.7から微調整
    });
